...
class Settings:
    ...
    def _detect_available_device() -> str:
        """
        自动检测可用的设备
        优先使用 MUSA GPU，其次 CUDA GPU，最后使用 CPU
        """
        try:
            import torch
    ...
    ALLOWED_IMAGE_EXTENSIONS: set = {
        ext.strip() for ext in os.getenv("ALLOWED_IMAGE_EXTENSIONS", ".jpg,.jpeg,.png,.bmp").split(",")
    }
