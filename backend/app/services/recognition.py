    ...
    def initialize(self):
        ...
        try:
            ...
            if self.device.type == 'musa':
                # MUSA GPU - MTCNN 检测使用 CPU，识别模型可以使用 MUSA
                try:
                    import torch_musa
                    if hasattr(torch, "musa") and torch.musa.is_available():
                        ...
                        logger.info("⚠️  MTCNN 在 MUSA 设备上不支持某些操作，将使用 CPU 进行人脸提取")
                        logger.info("✓  识别模型将使用 MUSA GPU 进行推理")
                    else:
            ...
            self.model = InceptionResnetV1(pretrained='vggface2').eval().to(self.device)  # 识别模型使用原始设备（可以是 MUSA）
            self._load_face_database()
            logger.info(f"识别模型已初始化 (MTCNN设备: {mtcnn_device}, 识别模型设备: {device_str})")
